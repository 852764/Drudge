...
- 如果 Provider 返回结构化的 `context_length_exceeded` 或等价上下文错误，Agent 会保留系统指令、最新用户回合和完整工具事务，压缩摘要后只重试当前模型请求一次；普通 `400`、裸 `413` 或已产生部分流式输出的错误不会被盲目重试，避免重复副作用。
- 启用存储时，摘要和近期消息事务化保存为上下文检查点；重启恢复只追加检查点之后的新消息，完整原始历史保留。
- 检查点提交失败时保留原工作上下文；检查点损坏时提示并回退原始历史。

会话分支、过期写入检测和中断事务修复详见[长任务上下文恢复](DURABLE_CONTEXT.md)。
